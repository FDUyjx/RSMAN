...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {"input_ids": batch[0],
                      ...
                      "ent_men_mask": batch[3],
                      'cls_mask': batch[4],
                      "ent_ner": batch[5],
                      "ent_pos": batch[6],
                      "ent_distance": batch[7],
                      "structure_mask": batch[8],
                      "label": batch[9],
                      "label_mask": batch[10],
                      }
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 or (
                ...
                if  global_step % 1000 == 0:
                    ...
                    logger.info({'learning_rate': learning_rate_scalar, 'step':global_step})
    ...
    return global_step, tr_loss / global_step, best_results
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        if preds is None:
            ...
            ent_masks = inputs["ent_men_mask"].detach().cpu().numpy()
            out_label_ids = inputs["label"].detach().cpu().numpy()
        else:
            preds = np.append(preds, logits.detach().cpu().numpy(), axis=0)
            ent_masks = np.append(ent_masks, inputs["ent_men_mask"].detach().cpu().numpy(), axis=0)
...
def load_and_cache_examples(args, tokenizer, evaluate=False, predict=False):
    ...
    all_ent_men_mask = torch.tensor([f.ent_men_mask for f in features], dtype=torch.float)
    all_cls_mask = torch.tensor([f.cls_mask for f in features], dtype=torch.float)
    ...
    dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids,
                            all_ent_men_mask, all_cls_mask, all_ent_ner, all_ent_pos, all_ent_distance,
...
def main():
    ...
    parser.add_argument("--warmup_ratio", default=0.1, type=float, help="Linear warmup ratio, overwriting warmup_steps.")
    parser.add_argument("--lr_schedule", default='constant', type=str, choices=['linear', 'constant'],
    ...
    if args.do_train:
        ...
        global_step, tr_loss, best_results = train(args, train_dataset, model, tokenizer)
