...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        sent_structure = structure_mask[5]
        attention_scores += sent_structure * (-1000000.0 * math.sqrt(self.attention_head_size))
...
class BertModel(BertPreTrainedModel):
    ...
    Examples::
        ...
        if self.config.is_decoder and encoder_hidden_states is not None:
            ...
            encoder_extended_attention_mask = (1.0 - encoder_extended_attention_mask) * -1000000.0
        else:
...
class BertForDocRED(BertPreTrainedModel):
    ...
    Examples::
        ...
        outputs = torch.relu(self.dim_reduction(outputs)) # shape:[bs, max_seq_len, reduced_dim]
        ...
        men_rep = torch.matmul(ent_men_mask.view(ent_men_mask.shape[0], -1, ent_men_mask.shape[-1]), outputs).view(ent_men_mask.shape[0],ent_men_mask.shape[1],ent_men_mask.shape[2],-1) # shape:[bs, max_ent_cnt, max_men_cnt, reduced_dim]

        cls_rep = torch.matmul(cls_mask, outputs) # shape:[batch_size,max_sent_cnt,reduced_dim]
        cls_rep = torch.mean(cls_rep,dim=1,keepdim=True)
        cls_info = cls_rep.unsqueeze(1).repeat(1, self.max_ent_cnt, self.num_labels, 1)

        # get relation-specific attention for each mention
        # [bs, max_ent_cnt*max_men_cnt,reduced_dim] * [reduced_dim,num_labels]
        men_attention = torch.matmul(nn.Tanh()(self.attention_weight(men_rep.view(ent_men_mask.shape[0],-1,self.reduced_dim))), self.attention_net.transpose(0,1).contiguous()) # shape:[bs, max_ent_cnt*max_men_cnt, num_labels]
        # add mask before softmax
        men_attention = men_attention.view(ent_men_mask.shape[0],ent_men_mask.shape[1],ent_men_mask.shape[2],-1)+men_mask # shape:[bs, max_ent_cnt, max_men_cnt, num_labels]
        ...
        ent_rep_spec = torch.matmul(men_attention,men_rep) # shape:[bs, max_ent_cnt, num_labels, reduced_dim]

        ent_rep_spec = torch.cat([ent_rep_spec,cls_info],dim=-1)
        ent_rep_spec = self.dropout(ent_rep_spec)

        # [bs, max_ent_cnt, num_labels, (1), reduced_dim] * [num_labels, self.reduced_dim, reduced_dim] -> [bs, max_ent_cnt, num_labels, (1), reduced_dim]
        logits = torch.matmul(ent_rep_spec.unsqueeze(3), self.classifier.expand(ent_rep_spec.shape[0],ent_rep_spec.shape[1],self.num_labels,self.reduced_dim*2,self.reduced_dim*2))

        # [bs, max_ent_cnt, (max_ent_cnt), num_labels, reduced_dim] * [bs, (max_ent_cnt), max_ent_cnt, num_labels, reduced_dim]
        logits = torch.mul(logits.squeeze(3).unsqueeze(2).repeat(1, 1, self.max_ent_cnt, 1, 1), ent_rep_spec.unsqueeze(1).repeat(1, self.max_ent_cnt, 1, 1, 1)).sum(dim=-1)+self.classifier_bais # shape:[bs, max_ent_cnt, max_ent_cnt, num_labels]

        loss_fct = BCEWithLogitsLoss(reduction='none')
        loss_all_ent_pair = loss_fct(logits.view(-1, self.num_labels), label.float().view(-1, self.num_labels)) # shape:[bs, max_ent_cnt, max_ent_cnt]
