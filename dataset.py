...
def docred_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if len(input_tokens) <= max_length:
            ...
            tok_to_sent = tok_to_sent[:max_length]
            tok_to_word = tok_to_word[:max_length]
            input_ids = tokenizer.convert_tokens_to_ids(input_tokens)
            attention_mask = [1] * len(input_ids)
        ...
        # cls feature
        max_sent_cnt = 25
        cls_mask = numpy.zeros((max_sent_cnt, max_length), dtype='int64')
        cls_cnt = 0
        for tok_idx in range(len(input_tokens)):
            if input_tokens[tok_idx] == tokenizer.cls_token and cls_cnt < max_sent_cnt:
                cls_mask[cls_cnt][tok_idx] = 1
                cls_cnt += 1
        ...
        for ent_idx, ent in enumerate(ents):
            for men_idx, mention in enumerate(ent):
                if men_idx>=max_men_cnt:
                    break
                for tok_idx in range(len(input_ids)):
                    if tok_to_sent[tok_idx] == mention['sent_id'] \
                        ...
                        ent_men_mask[ent_idx][men_idx][tok_idx] = 1
        ...
        structure_mask = numpy.zeros((6, max_length, max_length), dtype='float')
        ...
        for i in range(max_length):
            if attention_mask[i] == 0:
                break
            if type(tok_to_sent[i]) == str:
                cls_id = int(tok_to_sent[i])
                for j in range(max_length):
                    if tok_to_sent[j] is None:
                        continue
                    if (type(tok_to_sent[j]) == str and int(tok_to_sent[j]) != cls_id) or (
                            type(tok_to_sent[j]) == int and tok_to_sent[j] != cls_id):
                        structure_mask[5][i][j] = 1
        ...
        assert ent_men_mask.shape == (max_ent_cnt, max_men_cnt, max_length)
